...
namespace MRC.Framework.Mvc.Helpers
{
    ...
    public static class ExcelHelper
    {
        ...
        public static void ToExcel<T>(this List<T> list, string Title = "Excel Report", IList<string> HeaderNames = null)
        {
            ...
            using (ExcelPackage pck = new ExcelPackage())
            {
                ...
                for (int i = 0; i < Headings.Count(); i++)
                {
                    ...
                    else ws.Cells[2, i + 1].Value = HeaderNames[i];
                    if (Headings[i].PropertyType == typeof(DateTime?)
                        || Headings[i].PropertyType == typeof(DateTime)
                        )
                    {
                        ws.Column(i + 1).Style.Numberformat.Format = "mm/dd/yyyy hh:mm AM/PM";
                    }
                    ...
                    {
                        ws.Column(i + 1).Style.Numberformat.Format = "#,##0.0";
                    }
                }
                #endregion
                //Format the header
