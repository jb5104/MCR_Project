...
SELECT A.AD_CODE
	...
	--,B.AD_DEVICE_CODE
	--,C.DEVICE_CODE
	--,CONVERT(NUMERIC(18,2), ROUND(dbo.FN_TO_DISTANCE(@LATITUDE, @LONGITUDE, C.LATITUDE, C.LONGTITUDE,'KM'),2)) RADIUS /*소숫점 둘째 자리에서 반올림*/
	--, H.KEYWORD_CODE, H.KEYWORD_NAME, CONVERT(DATETIME, B.FR_DATE + ' ' + B.FR_TIME) , dbo.FN_TO_DISTANCE(@LATITUDE, @LONGITUDE, C.LATITUDE, C.LONGTITUDE,'KM'),CONVERT(DATETIME, B.TO_DATE + ' ' + CASE WHEN B.TO_TIME = '00:00' THEN '23:59:59' ELSE B.TO_TIME END),  c.*, G.*, E.*,  * 
 FROM T_AD A
...
WHERE 1=1
  ...
  AND dbo.FN_TO_DISTANCE(@LATITUDE, @LONGITUDE, C.LATITUDE, C.LONGTITUDE,'KM') <= @RADIUS
...
ORDER BY A.AD_CODE DESC -- CASE WHEN ISNULL(A.REQUEST_STORE_CODES,'') <> '' THEN 1 ELSE 0 END DESC, dbo.FN_TO_DISTANCE(@LATITUDE, @LONGITUDE, C.LATITUDE, C.LONGTITUDE,'KM')
